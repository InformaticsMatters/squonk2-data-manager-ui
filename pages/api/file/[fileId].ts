...
import { FixedSizeStreamTransform } from '../../../apiUtils/FixedSizeStreamTransform';
import { handleApiError } from '../../../apiUtils/handleApiError';
import { BadRequestError, InternalServerError } from '../../../apiUtils/HttpError';
...
/**
 * Restreams a specified file from DM API and potentially decompresses it or limits the size of
 * transfer on the fly. The restreaming consists of a source stream, a destination stream and a
 * couple of optional transforms. The source stream is the response from DM API, the output stream
 * is the response returned back to a client. The potential transforms take care of decompressing
 * and limiting the size of the returned response. The source is stream is piped through transforms
...
export default withApiAuthRequired(async (req, res) => {
  const { fileId, decompress, fileSizeLimit } = req.query;
