...
A user is a user of the Squonk Data Manager. They are defined in our Keycloak Single Sign On (SSO) system.
...
The Squonk Data Manager has the concept of an _Organisation_ (e.g. your company or university) and within that
Organisation you have _Units_ (e.g. a research group or department).
A _Project_ is where you can do some work, and _Projects_ belong to a _Unit_.
A _Project_  allows to isolate your work and restrict access to it to specific users.
Projects can be shared with users outside your organisation if you wish.
The _Unit_ is responsible for the usage of all its projects and will (at some stage) need to pay for that usage.

There is a _Default_ Organisation which all users can belong to, and within that _Default_ _Organisation_ you will be
given your own _Personal_ unit where you can create _Projects_.
...
Datasets are files that you can work with. You don't need to use datasets to do some work in Squonk Data Manager, but
...
As already mentioned a _Project_ is where you do your work, which can be kept private or shared with other users.
...
Each user has a unique unix user id and each project has an unique group id, with all users added to the project being
members of that group. This means that, with the file permissions describe above, users added to your project have
...
Currently the only application we have in Jupyter Notebooks. You can fire up a notebook to work with you files, and
...
To help illustrate this we'll use the [max-min-picker](https://discourse.squonk.it/t/job-max-min-picker/69) job as an example.
This allows a diverse subset of molecules to be selected from a set of input molecules.
...
A guide to creating jobs can be found [here](https://github.com/InformaticsMatters/virtual-screening/blob/main/DEVELOPER_GUIDE.md).
Creating jobs is a role for a developer, but using them is very easy.

Like applications, jobs run as Kubernetes pods. Some jobs are very simple and complete in seconds, whilst others can run
complex parallelised workflows.
...
Well, if you are an organisation deploying your own Squonk to your own Kubernetes cluster (or Informatics Matters is
doing this for you) then you are already paying for the CPU and memory resources being used.
If you are using our public evaluations site then we make _Evaluation_ tier projects available for free, but all data
in them is public so visible to any user. If you want to keep your data private you can subscribe your project to the
Bronze, Silver or Gold tiers, which differ most in the usage quotas, with Gold having the highest quota.
...
projection of what your current usage will cost by the end of your billing month, but of course this is only an approximation.
...
Also, some jobs might have significant IP value or use commerical software that has to be paid for so those can have
significant costs of execution.
