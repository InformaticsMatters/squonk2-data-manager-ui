...
import { AppApiOrganisationGetResponse } from "@/api/account-server/organisation/zod";
...
export const createScenarioFixtures = (subject: string) => {
  ...
  return {
    ...
    organisations: AppApiOrganisationGetResponse.parse({ count: 1, organisations: [organisation] }),
